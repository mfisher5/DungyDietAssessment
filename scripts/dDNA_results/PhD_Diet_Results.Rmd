...
index.sm.df <- as.data.frame(t(index.sm.mat)) %>%
  ...
  pivot_wider(names_from=taxon,values_from=DNAindex)
```
Bind small / regular tax together
```{r}
...
if(!knitting){
  index.df %>%
  write_csv(here(resultsdir,'lerayxt_r1-2_filtered2_eDNAindex.csv'))
   index.sm.df %>%
  write_csv(here(resultsdir,'lerayxt_r1-2_filtered2_SmallTaxa_eDNAindex.csv'))
   index.all.df %>%
  write_csv(here(resultsdir,'lerayxt_r1-2_filtered2_AllTaxa_eDNAindex.csv'))
}
```
