...
smdat <- read_csv(here(resultsdir, paste0(marker,"_","r",paste(run.nums,collapse="-"),"_sample_taxonomy_filtered3_SmallTaxa_uniqueTaxa.csv")))  
...
fig1b <- ggplot(crab_metadat, aes(x=CW_mm,y=CH_mm,color=fct_relevel(estuary, estuary_colors$estuary))) +
  geom_point() + facet_grid(cols=vars(fct_relevel(estuary, estuary_colors$estuary))) +
...
ggplot(crab_metadat, aes(x=CW_mm)) + 
...
Using **Taxa** instead of just **species** allows for for non-species-level IDs, as long as they are unique. 
...
-- NOT THIS ONE --- calculate eDNA index for all technical replicates (1 row = 1 crab x 1 replicate)
```{r eval=FALSE}
...
index.sm.mat <- eDNA_index(x=t(crab.smreads.mat))

index.sm.df <- as.data.frame(t(index.sm.mat)) %>%
  rownames_to_column("crab_id") %>% 
  pivot_longer(cols=2:(dim(index.sm.mat)[1]+1), names_to="taxon",values_to="DNAindex") %>%
...
index.all.mat <- eDNA_index(x=t(crab.allreads.mat))

index.all.df <- as.data.frame(t(index.all.mat)) %>%
  rownames_to_column("crab_id") %>% 
  pivot_longer(cols=2:(dim(index.all.mat)[1]+1), names_to="taxon",values_to="DNAindex") %>%
  left_join(alldat %>% dplyr::select(crab_id,site,estuary) %>% distinct(),by="crab_id") %>%
  left_join(site_names) %>%
  pivot_wider(names_from=taxon,values_from=DNAindex)
```
...
if(!knitting){
  index.df %>%
  write_csv(here(resultsdir,'lerayxt_r1-2_filtered2_sumtech-standardized_eDNAindex.csv'))
   index.sm.df %>%
  write_csv(here(resultsdir,'lerayxt_r1-2_filtered2_sumtech-standardized_SmallTaxa_eDNAindex.csv'))
   index.all.df %>%
  write_csv(here(resultsdir,'lerayxt_r1-2_filtered2_sumtech-standardized_AllTaxa_eDNAindex.csv'))
}
```
...
index.filter.df <- as.data.frame(t(index.mat)) %>%
  rownames_to_column("crab_id") %>% 
  pivot_longer(cols=2:(dim(index.mat)[1]+1), names_to="taxon",values_to="DNAindex") %>%
  left_join(dat %>% dplyr::select(crab_id,site,estuary) %>% distinct(),by="crab_id") %>%
  left_join(site_names) %>%
...
if(!knitting){
  index.filter.df %>%
  write_csv(here(resultsdir,'lerayxt_r1-2_filtered2_sumtech-standardized-noRare_eDNAindex.csv'))
   index.sm.filter.df %>%
  write_csv(here(resultsdir,'lerayxt_r1-2_filtered2_sumtech-standardized-noRare_SmallTaxa_eDNAindex.csv'))
   index.all.filter.df %>%
  write_csv(here(resultsdir,'lerayxt_r1-2_filtered2_sumtech-standardized-noRare_AllTaxa_eDNAindex.csv'))
}
```



# 2. Prey Diversity (Taxa)

## Alpha div

What is the overall number of unique taxa? **142**
```{r}
length(alldat_overall %>% pull(taxon) %>% unique())
```
```{r}
alldat_overall %>% group_by(phylum,class,order,family,genus,species) %>% distinct() %>% summarise(ncrab=length(unique(crab_id))) %>%
  arrange(phylum, class, order, family, genus) %>%
  write_csv(here('data','results','lerayxt_r1-2_filtered3_AllTaxa_unique_all_crab.csv'))
```
...
```{r}
alldat_sites <- get_unique_taxa(taxa.df=alldat, level="site")

richness.site <- alldat_sites %>% group_by(site) %>% summarise(richness=length(unique(taxon)),
...
plot.alpha <- allrichness.crab %>% mutate(dataset="All Taxa") %>%
  bind_rows(richness.crab %>% mutate(dataset="Multi-cellular Taxa")) %>%
  bind_rows(smrichness.crab %>% mutate(dataset="Unicellular Taxa")) %>%
...
p.alpha3 <- ggplot(richness.crab %>% filter(!is.na(stomach_fullness)), aes(x=as.factor(stomach_fullness),y=richness)) +
  geom_boxplot() +
  geom_jitter(width=0.1) +
  labs(y="",x="", title="Only Multi-cellular\n Taxa") +
  scale_y_continuous(limits=c(0,max(richness.crab$richness)+2), expand=c(0,0)) +
  theme_bw()
...
p.alpha4 <- ggplot(drichness.crab %>% filter(!is.na(stomach_fullness)), aes(x=as.factor(stomach_fullness),y=richness)) +
  geom_boxplot() +
  geom_jitter(width=0.1) +
  labs(y="",x="", title="Only Unicellular\n Taxa") +
...
p2 <- ggplot(plotdat2, aes(x=ncrab,y=nsp,color=estuary)) +
  geom_point( size=0.5) + geom_path() +
  scale_color_manual(values=c("gray30",estuary_colors$key[c(2,1,3)])) +
  labs(y="",x="number of crab", subtitle="only multi-cellular taxa") + theme_bw()
...
create summary data frames for all v. single-celled v. multi-cellular organisms
```{r}
site.bdiv.summary <- as.data.frame(bdiv) %>% rownames_to_column("site1") %>%
  pivot_longer(cols=2:(ncol(bdiv)+1), names_to="site2",values_to="jaccard") %>%
  mutate(taxa="All") %>%
  bind_rows(
    as.data.frame(bdiv2) %>% rownames_to_column("site1") %>%
      pivot_longer(cols=2:(ncol(bdiv)+1), names_to="site2",values_to="jaccard") %>%
      mutate(taxa="Multi-cellular")
  ) %>%
  bind_rows(
    as.data.frame(bdiv2b) %>% rownames_to_column("site1") %>%
      pivot_longer(cols=2:(ncol(bdiv)+1), names_to="site2",values_to="jaccard") %>%
      mutate(taxa="Unicellular")
  ) %>%
  mutate(comparison=paste0(site1, " - ", site2))

estuary.bdiv.summary <- as.data.frame(bdiv3) %>% rownames_to_column("site1") %>%
  pivot_longer(cols=2:(ncol(bdiv3)+1), names_to="site2",values_to="jaccard") %>%
  mutate(taxa="All") %>%
  bind_rows(
    as.data.frame(bdiv4) %>% rownames_to_column("site1") %>%
      pivot_longer(cols=2:(ncol(bdiv3)+1), names_to="site2",values_to="jaccard") %>%
      mutate(taxa="Multi-cellular")
  ) %>%
  bind_rows(
    as.data.frame(bdiv4b) %>% rownames_to_column("site1") %>%
      pivot_longer(cols=2:(ncol(bdiv3)+1), names_to="site2",values_to="jaccard") %>%
      mutate(taxa="Unicellular")
  ) %>%
  mutate(comparison=paste0(site1, " - ", site2))
...
Supp Figure: pairwise beta diversity (y axis) for sites, within / between estuaries (faceted), with different data sets colored.  
```{r}
p.bdive <- estuary.bdiv.summary %>%
  filter(site1 != site2) %>%
  ggplot(aes(x=comparison,y=jaccard,fill=taxa)) +
  geom_col(position="dodge") +
  geom_hline(aes(yintercept=0)) +
  scale_fill_grey(name="taxa") +
  ylim(c(0,1.0)) + labs(x="Estuary Pair",y="Beta Diversity") +
  theme_bw() +
  theme(axis.text.x=element_text(angle=45,hjust=1))
p.bdive
p.bdivs <- site.bdiv.summary %>%
  filter(site1 != site2) %>%
  # ggplot(aes(x=comparison,y=jaccard,fill=taxa)) +
  # geom_col(position="dodge") +
  ggplot(aes(x=taxa,y=jaccard)) +
  geom_boxplot() +
  geom_hline(aes(yintercept=0)) +
  scale_fill_grey(name="taxa") +
  ylim(c(0,1.0)) + labs(x="taxa",y="Beta Diversity", subtitle="Site Pairs") +
  theme_bw() +
  theme(axis.text.x=element_text(angle=45,hjust=1))
p.bdivs
```
...
if(!knitting){
  ...
  png(here('figs','figS_betadiv_byDataSet.png'), res=300,height=1000, width=1850)
  plot_grid(p.bdive+theme(plot.margin=margin(5.5,5.5,5.5,50)),p.bdivs, rel_widths=c(1,0.4), labels=c("(a)","(b)"))
  dev.off()
}
```
...
ggplot(aes(x=zorder,y=val,color=estuary,pch=estuary)) +
  ...
  facet_wrap(~metric,scales="free_y") +
  scale_color_manual(values=estuary_colors$key, name="estuary") +
  labs(x="Zeta order (samples)",y="", title="multi-cellular taxa only") +
  theme_bw()
zplot3
```
...
#### PERMANOVA for all taxa

First by **estuary**

```{r}
perm.allindex.indat <- index.all.filter.df %>% 
  dplyr::select(c(1,3,5:dim(index.all.filter.df)[2]))
```

```{r}
set.seed(55)
est.allindex.perm <- adonis2(perm.allindex.indat[,3:dim(perm.allindex.indat)[2]]~estuary, data=perm.allindex.indat, method="bray")
est.allindex.perm
```

The permanova is highly significant

Is the significance due to the differences in centroids or due to the dfferences in dispersion of samples in principal coordinate space of dissimilarity (PCoA)?
```{r}
set.seed(55)
allindex.indat.dist <- vegdist(x=perm.allindex.indat[,3:dim(perm.allindex.indat)[2]],method="bray")
est.allindex.perm.betadisper <- betadisper(d=allindex.indat.dist, group=perm.allindex.indat$estuary)

anova(betadisper(d=allindex.indat.dist, perm.allindex.indat$estuary))
# adonis2(dist(estuary.index.perm.betadisper$distances)~estuary.index.perm.betadisper$group)
```
Non-significant result from PERMDISP

And here's the PCoA from the PERMDISP.
```{r}
myplot.est.all <- gg_ordiplot_customLines(ord=est.allindex.perm.betadisper, groups=perm.allindex.indat$estuary, 
...
```{r}
permS.allindex.indat <- index.all.filter.df %>% 
  dplyr::select(c(1,4,5:dim(index.all.filter.df)[2]))
```

```{r}
set.seed(55)
site.allindex.perm <- adonis2(permS.allindex.indat[,3:dim(permS.allindex.indat)[2]]~site_name, data=permS.allindex.indat, method="bray")
site.allindex.perm
```
...
```{r}
set.seed(55)
site.allindex.perm.betadisper <- betadisper(d=allindex.indat.dist, group=permS.allindex.indat$site_name)

anova(betadisper(d=allindex.indat.dist, permS.allindex.indat$site_name))
# adonis2(dist(estuary.index.perm.betadisper$distances)~estuary.index.perm.betadisper$group)
```
...
And here's the PCoA from the PERMDISP.
```{r}
myplot.site.all <- gg_ordiplot_customLines(ord=site.allindex.perm.betadisper, groups=permS.allindex.indat$site_name, 
                                  groups_plotdiff="color,shape",hull=FALSE, ellipse=FALSE, label=FALSE, spiders=TRUE, 
                                  pt.size=2, line.alpha=0.5)
myplot.site.all$plot + 
  # scale_shape_manual(values=c(15,17, 16), name="Site") +
  # scale_color_manual(values=estuary_colors$key[c(2,1,3)], name="Estuary") +
  labs(x="PCoA Dim 1",y="PCoA Dim 2") +
  theme_bw() + theme(axis.text=element_blank(),axis.ticks=element_blank())
```
...
#### PERMANOVA for multicellular taxa

First by **estuary**

```{r}
perm.index.indat <- index.filter.df %>% 
  dplyr::select(c(1,3,5:dim(index.filter.df)[2]))
```

```{r}
set.seed(55)
est.index.perm <- adonis2(perm.index.indat[,3:dim(perm.index.indat)[2]]~estuary, data=perm.index.indat, method="bray")
est.index.perm
```
...
```{r}
set.seed(55)
index.indat.dist <- vegdist(x=perm.index.indat[,3:dim(perm.index.indat)[2]],method="bray")
est.index.perm.betadisper <- betadisper(d=index.indat.dist, group=perm.index.indat$estuary)
...
And here's the PCoA from the PERMDISP.
```{r}
myplot.est.multi <- gg_ordiplot_customLines(ord=est.index.perm.betadisper, groups=perm.index.indat$estuary, 
                                  groups_plotdiff="color,shape",hull=FALSE, ellipse=FALSE, label=FALSE, spiders=TRUE, 
                                  pt.size=2, line.alpha=0.5)
myplot.est.multi$plot + 
  scale_shape_manual(values=c(15,17, 16), name="Estuary") +
  scale_color_manual(values=estuary_colors$key[c(2,1,3)], name="Estuary") +
  labs(x="PCoA Dim 1",y="PCoA Dim 2") +
  theme_bw() + theme(axis.text=element_blank(),axis.ticks=element_blank())
...
```{r}
permS.index.indat <- index.filter.df %>% 
  dplyr::select(c(1,4,5:dim(index.filter.df)[2]))
```

```{r}
set.seed(55)
site.index.perm <- adonis2(permS.index.indat[,3:dim(permS.index.indat)[2]]~site_name, data=permS.index.indat, method="bray")
site.index.perm
```
...
```{r}
set.seed(55)
index.indat.dist <- vegdist(x=permS.index.indat[,3:dim(permS.index.indat)[2]],method="bray")
site.index.perm.betadisper <- betadisper(d=index.indat.dist, group=permS.index.indat$site_name)

anova(betadisper(d=index.indat.dist, permS.index.indat$site_name))
# adonis2(dist(estuary.index.perm.betadisper$distances)~estuary.index.perm.betadisper$group)
```
Significant result from PERMDISP: groups differ in dispersion 

```{r}
myplot.site.multi <- gg_ordiplot_customLines(ord=site.index.perm.betadisper, groups=permS.index.indat$site_name, 
                                  groups_plotdiff="color,shape",hull=FALSE, ellipse=FALSE, label=FALSE, spiders=TRUE, 
                                  pt.size=2, line.alpha=0.5)
myplot.site.multi$plot + 
  labs(x="PCoA Dim 1",y="PCoA Dim 2") +
  theme_bw() + theme(axis.text=element_blank(),axis.ticks=element_blank())
```

#### Save the PCoAs from the PERMDISP.

(a) has all data, (b) has only multicellular taxa
```{r echo=TRUE}
png(here('figs','fig3_PCoA-estuary.png'), res=200, height=600,width=1200)
p1 <- myplot.est.all$plot + 
  scale_shape_manual(values=c(15,17, 16), name="Estuary") +
  scale_color_manual(values=estuary_colors$key[c(2,1,3)], name="Estuary") +
  labs(x="PCoA Dim 1",y="PCoA Dim 2") +
  theme_bw() + theme(axis.text=element_blank(),axis.ticks=element_blank(),legend.position="none")
p2 <- myplot.est.multi$plot + 
  scale_shape_manual(values=c(15,17, 16), name="Estuary") +
  scale_color_manual(values=estuary_colors$key[c(2,1,3)], name="Estuary") +
  labs(x="PCoA Dim 1",y="PCoA Dim 2") +
  theme_bw() + theme(axis.text=element_blank(),axis.title.y=element_blank(), axis.ticks=element_blank())
plot_grid(p1,p2,labels=c("(a)","(b)"), rel_widths=c(0.65,1))
dev.off()
```
...
myplot.site.multi <- gg_ordiplot_customLines(ord=site.index.perm.betadisper, groups=permS.index.indat$site_name, 
                                  groups_plotdiff="color,shape",hull=FALSE, ellipse=FALSE, label=FALSE, spiders=TRUE, 
                                  pt.size=2, line.alpha=0.5)
p1 <- myplot.site.all$plot + 
  labs(x="PCoA Dim 1",y="PCoA Dim 2") +
  theme_bw() + theme(axis.text=element_blank(),axis.ticks=element_blank(), legend.position="none")
p2 <- myplot.site.multi$plot + 
  labs(x="PCoA Dim 1",y="PCoA Dim 2") +
  theme_bw() + theme(axis.text=element_blank(),axis.ticks=element_blank(), axis.title.y=element_blank(), legend.title=element_blank())

png(here('figs','figS_PCoA-site.png'), res=200, height=900,width=1400)
plot_grid(p1,p2,labels=c("(a)","(b)"), rel_widths=c(0.73,1))
dev.off()
```
#### save the input / output in an r object

```{r}
out.list <- list(
perm.index.indat, est.index.perm, est.index.perm.betadisper, permS.index.indat, site.index.perm, site.index.perm.betadisper)
names(out.list) <- c("permanova input matrix, eDNA index", "permanova obj","permdisp input matrix, dist from eDNA index","permdisp obj")

saveRDS(out.list, here('data','results','permanova_permdisp_eDNAindex-sumtech-standardized-noRare_multicellular.rds'))

out.list <- list(
perm.allindex.indat, est.allindex.perm, est.allindex.perm.betadisper, permS.allindex.indat, site.allindex.perm, site.allindex.perm.betadisper)
names(out.list) <- c("permanova input matrix, eDNA index", "permanova obj","permdisp input matrix, dist from eDNA index","permdisp obj")

saveRDS(out.list, here('data','results','permanova_permdisp_eDNAindex-sumtech-standardized-noRare_allTaxa.rds'))
```
...
plotdat.fo <- dat %>% mutate(plot.group=ifelse(is.na(phylum), class, phylum)) %>%
  mutate(plot.group=ifelse(plot.group=="Arthropoda" & class=="Insecta","Insecta",
                           ifelse(plot.group=="Arthropoda" & class!="Insecta","Arthropoda",plot.group))) %>%
...
## detritus groups
fo.sm <- smdat %>% mutate(plot.group="unicellular taxa") %>%
...
The groups with the highest median frequency of occurrence, greatest number of taxa:
```{r}
fo.hi <- plotdat.fo %>% group_by(plot.group) %>% 
  summarise(med.fo=median(pcrab), ntaxa=length(unique(taxon))) %>%
  filter(ntaxa > 3) %>%
  slice_max(order_by=med.fo,n=4, with_ties=FALSE) %>%
  left_join(dplyr::select(plotdat.fo,plot.group,taxon) %>% distinct(),by='plot.group') %>%
  left_join(bind_rows(dat,smdat), by='taxon') %>%
  filter(crab_id %in% dat$crab_id) %>%
  
  mutate(plot.subgroup=ifelse(plot.group=="unicellular taxa" & is.na(phylum), class,
                              ifelse(plot.group=="unicellular taxa" & !is.na(phylum),phylum,
                                     ifelse(plot.group=="Arthropoda",order,
                                            ifelse(plot.group=="Phaeophyceae", order,
...
plot2a <- 
  ...
  geom_boxplot() + geom_jitter(alpha=0.5,pch=4, width=0.05) +
...
```{r}
plot2b <-  fo.hi %>% 
  mutate(plot.group=ifelse(plot.group=="unicellular taxa", "unicellular\ntaxa", plot.group)) %>%
  ggplot(aes(x=plot.subgroup,y=pcrab)) +
  geom_boxplot() + geom_hline(aes(yintercept=-0.1)) +
  facet_wrap(~plot.group, scales="free_x", ncol=4) +
  scale_y_continuous(trans=squish_trans(from=40,to=70,factor=10),
                       breaks=c(0,10,20,30,40,70,75)) +
  geom_text_repel(aes(label=outlier), na.rm=TRUE,min.segment.length = 0.01,nudge_y=0.5, size=3) +
...
plotdat.fo %>%
  bind_rows(filter(fo.hi, plot.group=="single-celled organisms") %>% mutate(plot.group=plot.subgroup) %>%
              dplyr::select(-plot.subgroup, -outlier)) %>%
  group_by(plot.group) %>%
  summarise(median.fo=median(pcrab),mean.fo=mean(pcrab),sd_fo=sd(pcrab)) %>%
  write_csv(here('data','results','freqoccur_medians_runs1-2_lerayxt_presence.csv'))
```
...
```{r}
plotdat.fo %>% group_by(plot.group) %>% summarise(medFO=median(pcrab)) %>%
  mutate(plot.subgroup=NA) %>%
  bind_rows(fo.hi %>% group_by(plot.group,plot.subgroup) %>%
              summarise(medFO=median(pcrab))) %>%
  write_csv(here('data','results','TableS1_MedFOs.csv'))
```
...
FO% of crustaceans?
```{r}
keep_classes <- c("Malacostraca","Thecostraca","Copepoda","Ostracoda","Hexanauplia",
                  "Mystacocarida","Ichthyostraca","Cyclida",
                  "Branchiopoda","Cephalocarida","Remipedia","Tantulocarida","Priscansermarinidae")
```

Calculate frequency of occurrence of each taxon.
```{r}
crustacea.fo <- alldat %>% 
  filter(class %in% keep_classes) %>%
  group_by(taxon,class,rank) %>%
  summarise(ncrab=length(unique(crab_id))) %>%
  mutate(pcrab=(ncrab/total.crab)*100) 

crustacea.fo %>% pull(pcrab) %>% quantile()

with(crustacea.fo, table(rank))
```
Calculate frequency of occurrence of all crustacea.
```{r}
alldat %>% 
  filter(class %in% keep_classes) %>%
  mutate(subphylum="Crustacea") %>%
  group_by(subphylum) %>%
  summarise(ncrab=length(unique(crab_id))) %>%
  mutate(pcrab=(ncrab/total.crab)*100) 
```
...
bivalve.fo %>% pull(pcrab) %>% quantile()

with(bivalve.fo, table(rank))
```
Calculate frequency of occurrence of all crustacea.
```{r}
alldat %>% 
