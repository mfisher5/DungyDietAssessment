...
```{r}
# png(here('figs','figS_CW_CH_bysite.png'))
crab_metadat %>% 
  left_join(site_names) %>% pivot_longer(cols=c("CW_mm","CH_mm"),names_to="metric",values_to="mm") %>%
  group_by(site,metric) %>% summarise(mean_mm=mean(mm),sd_mm=sd(mm)) %>%
  mutate(metric=ifelse(metric=="CW_mm","Carapace Width","Carapace Height"))
...
ggplot(aes(x=mean_cw,y=mean_ch,color=fct_relevel(site_name, c("Kayak Pt 1","Kayak Pt 2","March Pt","NERR","Samish Isl","Samish Bay","Larrabee")))) + 
  geom_point() + 
  geom_errorbar(aes(ymin=mean_ch-sdev_ch,ymax=mean_ch+sdev_ch)) +
  geom_errorbarh(aes(xmin=mean_cw-sdev_ch,xmax=mean_cw+sdev_cw)) +
