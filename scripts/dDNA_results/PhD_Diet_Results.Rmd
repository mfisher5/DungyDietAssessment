...
What is the total number of unique taxa observed? **121**
```{r}
...
plot.alpha <- richness.crab %>%
  ...
  labs(y="N. Unique Taxa per crab",x="") +
...
p.alpha2 <- ggplot(richness.crab %>% filter(!is.na(stomach_fullness)), aes(x=as.factor(stomach_fullness),y=richness)) +
  geom_violin() +
  geom_jitter(width=0.1) +
  labs(y="N. Unique Taxa per Crab",x="Stomach Fullness") +
  scale_y_continuous(limits=c(0,max(richness.crab$richness)+2), expand=c(0,0)) +
  theme_bw()
p.alpha2
```
...
```{r}
plot2a <- 
  ggplot(plotdat.fo, aes(x=plot.group.num,y=pcrab)) +
...
fo.hi <- plotdat.fo %>% group_by(plot.group) %>% 
  ...
  left_join(plotdat.fo,by='plot.group') %>%
  left_join(dplyr::select(dat, taxon, class, order, family, genus) %>% distinct(), by='taxon')

fo.hi <- fo.hi %>%
  mutate(plot.subgroup=ifelse(plot.group=="Raphidophyceae",))
```
