...
```{r}
p.alpha2 <- ggplot(richness.crab %>% filter(!is.na(stomach_fullness)), aes(x=as.factor(stomach_fullness),y=richness)) +
  geom_boxplot() +
  geom_jitter(width=0.1) +
  labs(y="",x="", title="Without Detritus") +
  scale_y_continuous(limits=c(0,max(richness.crab$richness)+2), expand=c(0,0)) +
  theme_bw()
p.alpha3 <- ggplot(all.richness.crab %>% filter(!is.na(stomach_fullness)), aes(x=as.factor(stomach_fullness),y=richness)) +
  geom_boxplot() +
  geom_jitter(width=0.1) +
  labs(y="",x="", title="With Detritus") +
  scale_y_continuous(limits=c(0,max(richness.crab$richness)+2), expand=c(0,0)) +
  theme_bw()
...
```{r echo=TRUE}
png(here('figs','figS_alpha-div_site_fullness.png'), res=200, height=600,width=800)
...
```{r}
plot2a <- 
  ggplot(plotdat.fo, aes(x=plot.group,y=pcrab)) +
  ...
  theme_bw() + labs(x="Prey Group", y="Frequency of occurrence (FO%)") + theme(axis.text.x=element_text(angle=45, hjust=1,size=12,vjust=1.05),
  ...
  mutate(plot.subgroup=ifelse(plot.group=="single-celled organisms" & is.na(phylum), class,
                              ifelse(plot.group=="single-celled organisms" & !is.na(phylum),phylum,
                                     ifelse(plot.group=="Arthropoda",order,
                                            ifelse(plot.group=="Mollusca",family,phylum))))) %>%
...
```
to do: 
reorder x axis by group and median value; squish y axis; get rid of so many classes of single-celled organisms (all phylum NA are algae??)

```{r}
plot2b <- 
  ggplot(fo.hi, aes(x=plot.subgroup,y=pcrab, fill=plot.group)) +
  geom_boxplot() + geom_hline(aes(yintercept=-0.1)) +
  # geom_text_repel(aes(label=outlier), na.rm=TRUE,min.segment.length = 0.01,nudge_y=0.5, size=3) +
