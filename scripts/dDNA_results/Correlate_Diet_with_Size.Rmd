...
### common taxa: standardized reads CORRECT

standardize to proportions of total reads for the given crab, then generate a new value from a multinomial


first, get the proportion of each crab's total reads that belong to each species
```{r}
crabID.preads.mat <- decostand(x=crabID.reads.filter.mat,method="total",MARGIN=1)
```

then, get the average total reads per crab (for the 11 species we're including in the matrix)
```{r}
avg_reads_all <- round(median(rowSums(data_matrix)),0)
```

calculate reads per species using the average total read depth, by drawing from a multinomially distributed random number vector
```{r}
set.seed(555)
std.reads.df <- crabID.preads.mat %>%
  rownames_to_column("crab") %>%
  pivot_longer(cols=2:(dim(crabID.preads.mat)[2]+1), names_to="taxon",values_to="value") %>%
  group_by(crab) %>%
  nest() %>%
  summarise(new_counts=purrr::map(data, ~rmultinom(n=1,size=726, prob=.x$value))) %>%
  unnest_longer(new_counts, indices_include=FALSE) %>% arrange(crab,desc(new_counts)) %>%
  bind_cols(crabID.preads.mat %>%
              rownames_to_column("crab") %>%
              pivot_longer(cols=2:(dim(crabID.preads.mat)[2]+1), names_to="taxon",values_to="value") %>% 
              rename(crab2=crab) %>% arrange(crab2,desc(value))) %>%
...
```{r}
to_fix <- length(filter(std.reads.df, sp_reads==0 & value != 0) %>% pull(crab))
max_trials <- 1200

s <- 560
i <- 0
while(to_fix > 0 & i < max_trials){
  set.seed(s)
  std.reads.df %<>% mutate(sp_reads=ifelse(sp_reads==0 & value != 0, 
                                            rmultinom(n=1,size=avg_reads_all, prob=value),sp_reads))
  
  to_fix <- length(filter(std.reads.df, sp_reads==0 & value != 0) %>% pull(crab))
  s <- s+5
  i <- i + 1
  message(to_fix, " crab with zeros remaining.")
}
```

do you need to manually fix the remaining to sp_reads=1?
```{r}
filter(std.reads.df, sp_reads==0 & value != 0)

std.reads.df %<>% mutate(sp_reads=ifelse((crab=="CLAY_08" & taxon=="Chordariaceae"), 1,sp_reads))
```


make matrix of the expected number of reads
```{r}
std.reads.mat <- std.reads.df %>% 
  dplyr::select(crab,taxon,sp_reads) %>%
  pivot_wider(names_from="taxon",values_from="sp_reads", values_fill=0)
```
...
Plot the fitted values
```{r}
png(here(resultsdir,paste0('zoid_rawMeans_AllTaxa10plus_pReads_4chains50K.png')), res=300,height=2000,width=3000)
fitted_2_filter %>%
  group_by(taxon,CW_mm) %>%
  summarise(mean_cw=mean, upper_cw=lo,lower_cw=hi, n=length(unique(obs))) %>%
ggplot(aes(y=mean_cw,x=as.numeric(CW_mm))) +
...
png(here(resultsdir,'zoid_fittedVals_v_pReads_AllTaxa10plus_4chains50K.png'), res=300,height=4000,width=2000)
plot_grid(p1,p2,ncol=2)
dev.off()
```
...
fitted_2_ref <- fitted_2_filter %>%
  ...
  filter(CW_mm==min_cw) %>%
  dplyr::select(group,taxon,mean,median) %>% rename(ref_mean=mean,ref_median=median) %>%
...
```{r}
## plot for each taxon
png(here(resultsdir,paste0('zoid_relMeans_AllTaxa10plus_pReads_4chains50K.png')), res=300,height=2000,width=3000)
ggplot(fitted_2_ref, aes(y=scaled_mean,x=as.numeric(CW_mm))) +
    geom_point() + geom_hline(aes(yintercept=0)) +
    facet_wrap(~taxon, scales="free_y") + 
    labs(x="Carapace Width (mm)", y="Difference from 5mm CW") +
    theme_bw() +
    theme(axis.text.x=element_text(angle=45,hjust=1))
dev.off()
```
...
ggplot(cw_diffs.df, aes(x=CW_diff)) + 
  ...
  labs(x="Differences in Carapace Width (mm)", y="density") + theme_bw()
```
