...
Drop rare species (occur in < 3 crab) 
```{r}
lg.index.filter.mat <- dropspc(as.matrix(index.lg.df2[,5:dim(index.lg.df2)[2]]), minocc=2)
# add row names back on
rownames(lg.index.filter.mat) <- index.lg.df2$crab_id
lg.index.filter.mat <- lg.index.filter.mat[which(rowSums(lg.index.filter.mat) > 0),]

dim(lg.index.filter.mat); dim(index.lg.df2)
```

add carapace widths
```{r}
lg.index.filter.df <- as.data.frame(lg.index.filter.mat) %>%
  rownames_to_column(var="crab_id") %>%
  left_join(cw.df,by="crab_id")

n <- dim(lg.index.filter.df)[2]
```

run CCA
```{r}
cc1 <- cca(X=as.matrix(lg.index.filter.df[,2:(n-1)]), Y=lg.index.filter.df[,n, drop=FALSE])
plot(cc1)
```
Plot without *Sepulidae* and sites 50/27
```{r}
cc1.obj <- fortify(cc1)

cc1.sites <- cc1.obj %>% filter(score=="sites") %>% dplyr::select(label,CCA1,CA1) %>% 
  mutate(CCA1=as.numeric(CCA1), CA1=as.numeric(CA1)) %>%
  mutate(crab_id=as.character(lg.index.filter.df[,1])) %>%
...
Plot without sites 50/27, and *Serpulidae*

```{r}
ggplot(filter(cc1.sites, !(label %in% c("sit50","sit27"))),aes(x=CCA1,y=CA1,color=estuary)) + 
  geom_point() + 
  geom_text(data=filter(cc1.species, label!= "Serpulidae"),aes(x=CCA1,y=CA1, label=label), inherit.aes = FALSE) +
  geom_segment(data=cc1.biplot,aes(x=0,y=0,xend=cc1.biplot$CCA1*4,yend=cc1.biplot$CA1),
               arrow = arrow(length = unit(0.2, "cm")),
                         colour = "darkblue", inherit.aes = FALSE) +
  geom_text(data = cc1.biplot,aes_string(x = cc1.biplot$CCA1*3.5, y = cc1.biplot$CA1+0.3, label = "label"), 
...
Plot only with species labels for taxa on far ends of CCA1
```{r}
...
```{r}
p.cca.high <- ggplot(filter(cc1.sites, !(label %in% c("sit50","sit27"))),aes(x=CCA1,y=CA1,color=estuary)) + 
  geom_point() + 
  geom_text_repel(data=filter(cc1.species, label %in% highest_cca & label !="Serpulidae"),
                  aes(x=CCA1,y=CA1, label=label), 
                  min.segment.length=0.01,size=3, inherit.aes = FALSE) +
  geom_segment(data=cc1.biplot,aes(x=0,y=0,xend=cc1.biplot$CCA1*4,yend=cc1.biplot$CA1),
               arrow = arrow(length = unit(0.2, "cm")),
                         colour = "darkblue", inherit.aes = FALSE) +
  geom_text(data = cc1.biplot,aes_string(x = cc1.biplot$CCA1*3.5, y = cc1.biplot$CA1+0.3, label = "label"), 
...
p.cca.low <- ggplot(filter(cc1.sites, !(label %in% c("sit50","sit27"))),aes(x=CCA1,y=CA1,color=estuary)) + 
  geom_point() + 
  geom_text_repel(data=filter(cc1.species, label %in% lowest_cca & label !="Serpulidae"),
                  aes(x=CCA1,y=CA1, label=label), 
                  min.segment.length=0.01,size=3,inherit.aes = FALSE) +
  geom_segment(data=cc1.biplot,aes(x=0,y=0,xend=cc1.biplot$CCA1*4,yend=cc1.biplot$CA1),
               arrow = arrow(length = unit(0.2, "cm")),
...
plot_grid(p.cca.low+theme(legend.position="none"),p.cca.high, rel_widths=c(0.8,1))
```

```{r}
png(here('figs','Fig4_CCA_lgTax_v_CW.png'),res=300,height=1400,width=3000)
plot_grid(p.cca.low+theme(legend.position="none"),p.cca.high + theme(axis.title.y=element_blank(),axis.text.y=element_blank()), rel_widths=c(0.8,1), labels=c("(a)","(b)"))
dev.off()
```
...
Drop rare species (occur in < 3 crab) 
```{r}
sm.index.filter.mat <- dropspc(as.matrix(index.sm.df2[,5:dim(index.sm.df2)[2]]), minocc=2)
# add row names back on
rownames(sm.index.filter.mat) <- index.sm.df2$crab_id
sm.index.filter.mat <- sm.index.filter.mat[which(rowSums(sm.index.filter.mat) > 0),]

dim(sm.index.filter.mat); dim(index.sm.df2)
```

add carapace widths
```{r}
sm.index.filter.df <- as.data.frame(sm.index.filter.mat) %>%
  rownames_to_column(var="crab_id") %>%
  left_join(cw.df,by="crab_id")

n <- dim(sm.index.filter.df)[2]
```

run CCA
```{r}
cc1 <- cca(X=as.matrix(sm.index.filter.df[,2:(n-1)]), Y=sm.index.filter.df[,n, drop=FALSE])
plot(cc1)
```
...
cc1.sites <- cc1.obj %>% filter(score=="sites") %>% dplyr::select(label,CCA1,CA1) %>% 
  mutate(CCA1=as.numeric(CCA1), CA1=as.numeric(CA1)) %>%
  mutate(crab_id=as.character(sm.index.filter.df[,1])) %>%
  left_join(dplyr::select(index.sm.df2, crab_id, site, estuary) %>% distinct(), by="crab_id")
...
Plot without site 32, and *Ditylum brightwelliie*

```{r}
ggplot(filter(cc1.sites, !(label %in% c("sit32"))),aes(x=CCA1,y=CA1,color=estuary)) + 
  geom_point() + 
  geom_text(data=filter(cc1.species, label!= "Ditylum brightwellii"),aes(x=CCA1,y=CA1, label=label), inherit.aes = FALSE) +
  geom_segment(data=cc1.biplot,aes(x=0,y=0,xend=cc1.biplot$CCA1*4,yend=cc1.biplot$CA1),
               arrow = arrow(length = unit(0.2, "cm")),
                         colour = "darkblue", inherit.aes = FALSE) +
  geom_text(data = cc1.biplot,aes_string(x = cc1.biplot$CCA1*3.5, y = cc1.biplot$CA1+0.3, label = "label"), 
...
```{r}
p.cca.high <- ggplot(filter(cc1.sites, !(label %in% c("sit32"))),aes(x=CCA1,y=CA1,color=estuary)) + 
  geom_point() + 
  geom_text_repel(data=filter(cc1.species, label %in% highest_cca & label !="Ditylum brightwellii"),
                  aes(x=CCA1,y=CA1, label=label), 
                  min.segment.length=0.01,size=3, inherit.aes = FALSE) +
  geom_segment(data=cc1.biplot,aes(x=0,y=0,xend=cc1.biplot$CCA1*4,yend=cc1.biplot$CA1),
               arrow = arrow(length = unit(0.2, "cm")),
...
p.cca.low <- ggplot(filter(cc1.sites, !(label %in% c("sit32"))),aes(x=CCA1,y=CA1,color=estuary)) + 
  geom_point() + 
  geom_text_repel(data=filter(cc1.species, label %in% lowest_cca & label !="Ditylum brightwellii"),
                  aes(x=CCA1,y=CA1, label=label), 
                  min.segment.length=0.01,size=3,inherit.aes = FALSE) +
  geom_segment(data=cc1.biplot,aes(x=0,y=0,xend=cc1.biplot$CCA1*4,yend=cc1.biplot$CA1),
               arrow = arrow(length = unit(0.2, "cm")),
...
```{r}
png(here('figs','Fig4_CCA_smTax_v_CW.png'),res=300,height=1400,width=3000)
plot_grid(p.cca.low+theme(legend.position="none"),p.cca.high + theme(axis.title.y=element_blank(),axis.text.y=element_blank()), rel_widths=c(0.8,1), labels=c("(a)","(b)"))
dev.off()
```
...
Drop rare species (occur in < 10 crab) 
```{r}
crabID.index.filter.mat <- dropspc(as.matrix(index.df2[,5:dim(index.df2)[2]]), minocc=)
# add row names back on
...
ggplot(cc1.sites,aes(x=CCA1,y=CA1,color=estuary)) + 
  ...
  geom_text_repel(data=filter(cc1.species, label %in% highest_cca),
                  aes(x=CCA1,y=CA1, label=label), 
                  min.segment.length=0.01,nudge_y=1,inherit.aes = FALSE) +
  ...
  geom_text_repel(data=filter(cc1.species, label %in% lowest_cca) %>% mutate(label2=paste0("italic('",label,"')")),
                  ...
                  min.segment.length=0.01, nudge_y=-1,inherit.aes = FALSE,parse=TRUE) +
...
```{r}
png(here('figs','Fig4_CCA_AllTaxa_v_CW_label90CI.png'), res=300,width=2100,height=1500)
ggplot(cc1.sites,aes(x=CCA1,y=CA1,color=estuary)) + 
  geom_point() + 
  geom_text_repel(data=filter(cc1.species, label %in% highest_cca),
                  aes(x=CCA1,y=CA1, label=label), 
