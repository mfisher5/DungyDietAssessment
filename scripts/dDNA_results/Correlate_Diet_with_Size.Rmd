...
```{r}
if(!knitting){
  png(here('figs','figS_CW_v_BCdist_ptsize.png'), res=300,width=1800,height=1200)
reg.in %>% 
  unite("crabs_compared",crab_id,crab_id2, remove=FALSE) %>%
  group_by(CW_diff, BCdistance) %>% summarise(n=length(unique(crabs_compared))) %>%
ggplot(aes(x=CW_diff,y=BCdistance, size=n)) +
...
For a beta distribution, we need to subtract a tiny value from values that equal '1'
```{r}
reg.in2 <- reg.in %>% mutate(BCdistance=ifelse(BCdistance==1, 0.99999999999, BCdistance))
...
```
```{r}
plot(fitted(cw.beta.ml),
     residuals(cw.beta.ml))

qqnorm(residuals(cw.beta.ml, type=""))
```

```{r}
plot(fitted(cw.beta.bc),
     residuals(cw.beta.bc))
```

```{r}
png(here('figs','figS_betareg_diagnostics.png'))
par(mfrow=c(3,2))
