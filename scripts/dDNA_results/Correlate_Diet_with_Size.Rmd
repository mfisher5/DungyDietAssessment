...
ggplot(aes(y=mean_cw,x=as.numeric(CW_mm))) +
  ...
  geom_text(aes(y=0.0015,x=as.numeric(CW_mm),label=n)) +
...
saveRDS(cc1, here('data','results','CCA_AllTaxa10plus_v_CW_2023-11-20.rds'))

}
```


read in the zoid data frame and combine plots
```{r}
fitted_2_ref2 <- read_csv(here(resultsdir,paste0('zoidFittedValsFiltered_preads_AllTaxa10plus_4ch5KiOD_2023-11-22.csv'))) %>%
 # set the reference according to minimum CW in which that taxon was detected
  group_by(taxon) %>% mutate(min_cw=min(CW_mm)) %>% ungroup() %>%
  filter(CW_mm==min_cw) %>%
  dplyr::select(group,taxon,mean,median,lo,hi) %>% rename(ref_mean=mean,ref_median=median,ref_lo=lo,ref_hi=hi) %>%
...
p.zoid <- ggplot(fitted_2_ref, aes(y=scaled_mean,x=as.numeric(CW_mm))) +
    geom_point() + geom_hline(aes(yintercept=0)) +
    facet_wrap(~taxon, 
               ncol=2,dir="v",scales="free_y") + 
    labs(x="Carapace Width (CW; mm)", y="Difference from minimum CW") +
...
p.cca <- plot.99ci + theme(legend.position=c(.82,.85))


if(!knitting){
  
png(here('figs','Fig4ab_CCA_Zoid_AllTaxa10plus_v_CW_label99CI.png'), res=300,width=2800,height=1400)
plot_grid(p.cca, p.zoid, labels=c("(a)","(b)"), rel_widths=c(1, 0.9))
dev.off()
