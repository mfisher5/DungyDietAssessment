...
fit_2_prey <- fit_zoid(formula = y ~ CW_mm, 
                      ...
                      data_matrix = as.matrix(data_matrix)/1000,
                       overdispersion = TRUE,
                       chains=1,
                       iter=4000,overdispersion_sd = 0.1)
```


save data
```{r}
saveRDS(zoidIN.reads,here(resultsdir,'zoidIN_nReads_allTaxa10plus_by_CW.rds'))
```
...
crabID.CLASSreads.mat <- dropspc(crabID.CLASSreads.mat, minocc=7)
...
## remove rare families
crabID.PHYreads.mat <- dropspc(crabID.PHYreads.mat, minocc=4)
...
fit_5_prey <- fit_zoid(formula = y ~ CW_mm, 
                      ...
                      data_matrix = as.matrix(data_matrix)/1000,
                       ...
                       iter=500,
                      overdispersion_sd = 0.1)   # for testing; change to 5000
```
...
Drop rare species (occur in < 5 crab) 
```{r}
crabID.index.filter.mat <- dropspc(as.matrix(index.df2[,5:dim(index.df2)[2]]), minocc=5)
