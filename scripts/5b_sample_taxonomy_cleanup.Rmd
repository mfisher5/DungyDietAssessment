...
I did leave in some of the "too small" organisms, add those to the manual filter.
```{r}
too_small_dat <- unqdat %>% filter(phylum %in% c("Bacillariophyta","Rotifera") |
                            class %in% c("Dinophyceae","Chlorophyceae"))
too_small_filter <- dplyr::select(too_small_dat, taxon, rank) %>%
  mutate(new_rank="NA",new_taxon="too small") %>% distinct()

manual_filter %<>% filter(!(new_taxon %in% too_small_filter$taxon)) %>%
...
```{r}
write_csv(unqdat_filter2, (here(outdir, paste0(marker,"_","r",paste(run.nums,collapse="-"),"_sample_taxonomy_filtered2_uniqueTaxa.csv"))))
```

Save a summary of the final filtered taxonomy
```{r}
unq_filter2_summary <- unqdat_filter2 %>%
  group_by(taxon,rank,kingdom,phylum,class,order,family,genus,species) %>%
  summarise(n.crab=length(unique(crab_id)),
            n.sites=length(unique(site)),
            sites=paste0(unique(site),collapse=","),
            miseqruns=paste0(unique(MiSeqRun),collapse=","))
write_csv(unq_filter2_summary, (here('data','results', paste0(marker,"_","r",paste(run.nums,collapse="-"),"_sample_taxonomy_filtered2_uniqueTaxaSUMMARY.csv"))))
```


### "too small" dataset

I want to create a "rotifers / algae / diatoms / protists" category that could potentially represent ingestion of detritus; Galloway et al. included detritus in their feeding experiments for Dungeness crab instars. This does *not* include fungi, amoebas, or bacteria

 **First, re-run lines 1-94 above**. Re-read in the manual filter data
```{r}
blast.small <- blast.dat.filter %>% filter(!is.na(rank))

manual_filter <- readxl::read_excel(path=here('data','results','lerayXT_r1-2_sample_taxonomy_filtered_uniqueTaxaSUMMARY.xlsx'),sheet='lerayXT_r1-2_final')
```

Add back in the small taxa, from either the manual filter dataset or from the list of taxa that I filtered out in the script above. 
```{r}
too_small_taxa <- filter(manual_filter,new_taxon=="too small")
too_small_phyla <- c("Bacillariophyta","Rotifera","Foraminifera")
too_small_class <- c("Dinophyceae","Chlorophyceae","Cryptophyceae")
too_small_genus <- c("Nannochloris","Micromonas","Pinnularia","Chattonellaceae","Chlamydomonadaceae","Chloropicaceae","Paralia","Tetraselmis")

blast.small %<>% 
  filter(phylum %in% too_small_phyla | class %in% too_small_class | genus %in% too_small_genus | taxon %in% too_small_taxa$taxon)
``` 

clean up non-unique taxa within each crab
```{r}
# species-level IDs
spdat <- blast.small %>% filter(rank=="species")

# IDs above species-level
nonspec_dat <- blast.small %>% 
  filter(rank != "species") %>%
  mutate(dup_sample=crab_id)
...
blast.small.unq <- nonspec_dat %>% dplyr::select(-data) %>% unnest(c(crab_id,data.filter)) %>% dplyr::select(-dup_sample) %>%
  bind_rows(spdat)
```

Save!
```{r}
write_csv(blast.small.unq %>% dplyr::select(-c(19:36)) %>% mutate(new_taxon="too small",new_rank=NA), (here(outdir, paste0(marker,"_","r",paste(run.nums,collapse="-"),"_sample_taxonomy_SmallTaxa_uniqueTaxa.csv"))))
```
