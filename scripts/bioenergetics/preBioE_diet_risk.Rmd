...
scale points by the number of taxa at that exact value. only plot values from prey present in two or more crab
```{r}
...
write_csv(risk.dat, here(bdir,'OArisk_runs1-2_lerayxt_2023-11-12.csv'))
  ...
  risk.dat %>% dplyr::select(new_taxon,pcrab,eindex.rescaled,std_ed,crab.risk) %>%
...
png(here('../../','figs','fig5b_crab_OArisk_all.png'),res=300,height=1800,width=2200)

risk.dat %>%
  left_join(redo_colors,by=c("new_taxon"="label")) %>%
  ggplot(aes(x=std_survival_scalar,y=std_ed, color=crab.risk)) +
  geom_point(size=2) +
  geom_text_repel(aes(label=new_taxon), fontface='italic', min.segment.length = 0.01, max.overlaps=15, col=redo_colors$labelcol) +
  scale_color_viridis_c(option="H",begin=0.4, end=0.8, name="Crab Risk") +
  labs(x="Crab Exposure Index", y="Crab Sensitivity") + theme_bw()
...
plot_grid(p.risk2 + theme_bw() + theme(axis.text.x=element_text(angle=25, hjust=1), legend.box="horizontal"),
          p.risk3 + theme_bw() + theme(axis.text.x=element_text(angle=25,hjust=1)),
