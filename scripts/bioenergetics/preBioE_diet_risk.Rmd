...
```{r}
risk.dat <- eindex.dat %>%
  left_join(ed.dat %>% dplyr::select(-functional_group,-ncrab), by=c("new_taxon")) %>%
...
risk.dat %>%
  ggplot(aes(x=std_survival_scalar,y=std_ed, color=crab.risk)) +
  geom_point(size=2) +
  geom_text_repel(aes(label=new_taxon), fontface='italic', min.segment.length = 0.01, max.overlaps=15) +
  scale_color_viridis_c(option="H",begin=0.4, end=0.8, name="Crab Risk") +
  labs(x="Crab Exposure Index", y="Crab Sensitivity") + theme_bw()
