...
```{r}
blast_filenames     <- c('hash_key_clean_blast_2022-11-03.fasta')
```
<br>

Output files
```{r}
consensus_out <- 'lerayXTblast_multipleIDs_taxonomy.csv'
...
```
```{r}
write_csv(multi_taxids_df, here(outdir, paste0(locus,'_r',run.num,'_multipleIDs_staxids_lca-input.csv')))
```
<br>
...
<br>
```{r}
write_csv(multi_seqids_df, here(outdir, paste0(locus,'_r',run.num,'_blast_multipleIDs_multrows_lca-input.csv')))
```
<br>
...
write.csv(missing_taxonomy, here(outdir, paste0(locus,'_r',run.num,'_missing_blast_taxonomy_by-sample.csv')), row.names = FALSE)
```
```{r echo=FALSE}
...
```{r}
write.csv(sample_taxonomy, here(outdir, paste0(locus,'_r',run.num,"_sample_blast_lca_taxonomy.csv")), row.names=FALSE)
```
<br>
