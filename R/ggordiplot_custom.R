#' Plot with Ellipses, Hulls, Spiders, with some custom features added
#'
...
#'
gg_ordiplot_customLines <- function(ord, groups, groups_plotdiff="color", scaling = 1, choices = c(1,2), kind = c("sd", "se", "ehull"), conf=NULL, show.groups="all", ellipse = TRUE, label = FALSE, hull = FALSE, spiders = FALSE, pt.size = 3, plot=TRUE, line.alpha=1) {
  ...
  xlab <- axis.labels[1]
  ylab <- axis.labels[2]
  if(groups_plotdiff=="color"){
    plt <- ggplot2::ggplot() +
      geom_point(data=df_ord, aes(x=x, y=y, color=Group), size = pt.size) +
      xlab(xlab) + ylab(ylab)
  } else if(groups_plotdiff=="shape"){
    plt <- ggplot2::ggplot() +
      geom_point(data=df_ord, aes(x=x, y=y, pch=Group), size = pt.size) +
      xlab(xlab) + ylab(ylab)
  } else if(groups_plotdiff=="color,shape"){
    plt <- ggplot2::ggplot() +
      geom_point(data=df_ord, aes(x=x, y=y, pch=Group, color=Group), size = pt.size) +
      xlab(xlab) + ylab(ylab)
  }else(stop("ERROR: choose to plot groups by color, shape, or both [argument:color,shape]"))
  
  # Add ellipses.
  if (ellipse == TRUE) {
    plt <- plt + geom_path(data = df_ellipse, aes(x=x, y=y, color=Group), alpha=line.alpha, show.legend = FALSE)
  }
  ...
  # Add hulls.
  if (hull == TRUE) {
    plt <- plt + geom_path(data=df_hull, aes(x=x, y=y, color=Group), alpha=line.alpha,  show.legend = FALSE)
  }
  
  # Add spiders.
  if (spiders == TRUE) {
    plt <- plt + geom_segment(data=df_spiders, aes(x=cntr.x, xend=x, y=cntr.y, yend=y, color=Group), alpha=line.alpha, show.legend = FALSE)
  }
